...
const TVItem = () => {
  ...
  function fetchTVShow() {
    return axios.get(
      `tv/${params.tvId}?api_key=${API_KEY}&append_to_response=videos`
    );
  }
  const { data, isLoading } = useQuery(
    ["tvShow", params.tvId],
    async () => await fetchTVShow(),
    {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await axios.get(
        `tv/${params.tvId}/reviews?api_key=${API_KEY}&language=en-US&page=1`
      );
                  ...
                  <div className="movie__reviews__row">
                    ...
                    <div className="movie__reviews__row__right">
                      ...
                      <p className="movie__reviews__row__right__content">
                        {reduceDescription(item.content, 6000)}
                      </p>
                    </div>
                  </div>
                ))}
